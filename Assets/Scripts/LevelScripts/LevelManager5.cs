        ...
        switch (type)
        {
            case BubbleType.Correct:
                StartCoroutine(AnswerSequence(
                    new string[] { "いつもメモを取るようにしています。" },
                    new string[] { "なるほど。それはいい習慣ですね。" },
                    true, chosenBubble));
                break;

            case BubbleType.False1:
                StartCoroutine(AnswerSequence(
                    new string[] { "勘でなんとかします。" },
                    new string[] { "それは少し不安ですね。" },
                    false, chosenBubble));
                break;

            case BubbleType.False2:
                StartCoroutine(AnswerSequence(
                    new string[] { "寝ると思い出します。" },
                    new string[] { "勤務中に寝るのはやめてください。" },
                    false, chosenBubble));
                break;

            default:
                StartCoroutine(AnswerSequence(
                    new string[] { "あとで思い出します。" },
                    new string[] { "一番信用できません。" },
                    false, chosenBubble));
                break;
        }
    }
