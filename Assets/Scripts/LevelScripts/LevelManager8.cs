        ...
        switch (type)
        {
            case BubbleType.Correct:
                StartCoroutine(AnswerSequence(
                    new string[] { "チームに貢献し、会社の成長に関わりたいと考えています。" },
                    new string[] { "明確で前向きな動機ですね。" },
                    true, chosenBubble));
                break;

            case BubbleType.False1:
                StartCoroutine(AnswerSequence(
                    new string[] { "なんとなく頑張ろうと思います。" },
                    new string[] { "。。つまり、ノープランですね。" },
                    false, chosenBubble));
                break;

            case BubbleType.False2:
                StartCoroutine(AnswerSequence(
                    new string[] { "入社したら、流れに身を任せて泳ぎます。" },
                    new string[] { "ここは水族館ではありませんよ。" },
                    false, chosenBubble));
                break;

            default:
                StartCoroutine(AnswerSequence(
                    new string[] { "全部分かってるフリをして、バレないことを祈ります。" },
                    new string[] { "それはすぐに破綻しますね。" },
                    false, chosenBubble));
                break;
        }
    }
