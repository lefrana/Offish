        ...
        switch (type)
        {
            case BubbleType.Correct:
                StartCoroutine(AnswerSequence(
                    new string[] { "知り合いから紹介してもらいました。" },
                    new string[] { "なるほど。それは安心できますね。" },
                    true, chosenBubble));
                break;

            case BubbleType.False1:
                StartCoroutine(AnswerSequence(
                    new string[] { "夢に出てきたからです。" },
                    new string[] { "。。現実の話をしましょう！" },
                    false, chosenBubble));
                break;

            case BubbleType.False2:
                StartCoroutine(AnswerSequence(
                    new string[] { "家から近かったので。" },
                    new string[] { "まあ、確かに海の近くではありますが。。" },
                    false, chosenBubble));
                break;

            default:
                StartCoroutine(AnswerSequence(
                    new string[] { "奇跡的に見つけました。" },
                    new string[] { "運に頼りすぎじゃないですか？" },
                    false, chosenBubble));
                break;
        }
    }
