        ...
        switch (type)
        {
            case BubbleType.Correct:
                StartCoroutine(AnswerSequence(
                    new string[] { "会社の考え方に共感しました。" },
                    new string[] { "それは大事な理由ですね。" },
                    true, chosenBubble));
                break;

            case BubbleType.False1:
                StartCoroutine(AnswerSequence(
                    new string[] { "運命だと思いました。" },
                    new string[] { "少しロマンチックすぎませんか？" },
                    false, chosenBubble));
                break;
            ...
            case BubbleType.False2:
                StartCoroutine(AnswerSequence(
                    new string[] { "有名だったので。" },
                    new string[] { "。。。<i>（皆さん、だいたい同じ理由ですね。。）</i>" },
                    false, chosenBubble));
                break;

            default:
                StartCoroutine(AnswerSequence(
                    new string[] { "いろいろな理由があります。" },
                    new string[] { "一つでいいので、教えてください！" },
                    false, chosenBubble));
                break;
        }
    }
